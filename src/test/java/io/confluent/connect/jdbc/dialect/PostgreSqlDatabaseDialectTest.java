...
public class PostgreSqlDatabaseDialectTest extends BaseDialectTest<PostgreSqlDatabaseDialect> {
  ...
  public void commitOnPreparedStatementClose() throws SQLException {
    ...
    when(mockConnection.prepareStatement(eq(query))).thenReturn(mockPreparedStatement);
    when(mockPreparedStatement.getConnection()).thenReturn(mockConnection);

    PreparedStatement stmtProxy = dialect.createPreparedStatement(mockConnection, query);
